  ...
  return (
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-40 px-3 pb-3">
        <div className="bg-white/90 backdrop-blur-xl border border-purple-100 shadow-[0_-4px_40px_rgba(124,58,237,0.12)] rounded-2xl max-w-md mx-auto">
          <div className="flex items-center justify-around px-1 py-2 relative">
              ...
              return (
                <Link key={href} href={href} className="flex-1">
                  <div className={cn(
                    ...
                    <div className={cn("p-1.5 rounded-xl transition-all", active && "bg-violet-50")}>
                      <Icon size={20} strokeWidth={active ? 2.5 : 2} />
                    </div>
                    <span className={cn("text-[10px] font-medium", active ? "text-violet-600" : "text-gray-400")}>{label}</span>
                    {active && <div className="w-1 h-1 rounded-full bg-violet-500" />}
                  </div>
                </Link>
              );
            })}
            ...
            <div className="flex-1 flex justify-center -mt-7">
              <button
                ...
                className="relative w-16 h-16 rounded-full bg-gradient-to-br from-violet-600 via-blue-600 to-cyan-500 flex items-center justify-center shadow-[0_4px_24px_rgba(124,58,237,0.45)] hover:shadow-[0_8px_40px_rgba(124,58,237,0.6)] transition-all duration-300 hover:scale-110 active:scale-95 glow-animate border-4 border-white"
              >
